...
// Old GCC (< 10) can't handle static constexpr aggregate init with
// template-dependent values, and Prefetch is a no-op on non-Clang
// compilers, so skip the entire prefetch block.
#if !(defined(__GNUC__) && !defined(__clang__) && !PROTOBUF_GNUC_MIN(10, 0))
